...
// Parameters at contructions/instanciation
	...
	public :
		...
		#ifdef DEBUG
			Buffered_Array<std::complex<FloatType>> np_FFT_kernel_p ;
			Buffered_Array<std::complex<FloatType>> np_FFT_kernel_q ;
			std::complex<FloatType>* p_FFT_kernel_p ;
			std::complex<FloatType>* p_FFT_kernel_q ;
			
			uint64_t execute_time;
		#endif
		// Contructor
		Time_Quad(uint64_t L_kernel = 2,uint64_t L_data = 16, 
