...
py::class_<FastConvolution<OutputType, KernelType, DataType, ComplexType>>( m , "FastConvolution_"#OutputType"_"#KernelType"_"#DataType"_"#ComplexType )\
	.def(py::init<uint64_t, uint64_t, uint32_t, std::string , int>(), py::arg("l_kernel") = 2, py::arg("l_data")=16, py::arg("l_FFT")= 8, py::arg("convolution_type")="fft" , py::arg("n_threads") = 4) \
	...
	.def_readwrite("output_valid", &FastConvolution<OutputType,KernelType, DataType, ComplexType>::np_output_valid) \
