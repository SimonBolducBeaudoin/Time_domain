...
# LOOP PARAMETERS 
n_measures = 325 if not test else 5 ;
...
def plot_Hist_1D_difference( Hs , Vdc , axis = 0 , x_label="" , y_label="", TR_comment="" ,height = 7 , Ratio_hw = 1.62 ):
    ...
    fig.set_size_inches( Ratio_hw*height , height )
    fig.text(0.74, 0.915, TR_comment , fontsize=9)
    for H0, H1, vdc in zip(Hs[0:-1:2,i],Hs[1:-1:2,i],Vdc[1:-1:2]) :
        label = "{:2.3F} [V]".format(vdc)
...
# plot_Hist_1D_difference( Hs , Vdc )


# histogram 2D

# figure();
# imshow(Hs[0,0].get(),cmap='Reds', interpolation='none'  );
# colorbar();
