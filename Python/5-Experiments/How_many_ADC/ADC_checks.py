...
#############################
n_aquisitions = 2*16*12 ; # The number of aquisition made with Guzik
l_data= 2**28 ; # The length of one aquisition
...
print("Experience over : \n \t Estimated time [s] {:04.2F} \n \t Real time {:04.2F} [s] \n ").format( n_aquisitions*Estimated_time_per_loop , t_end-t_begin )
...
def plot_Hist_1D_in_subfig( HS , labels , x_label="bin#" , y_label="Counts", TR_comment="" ,height = 10 , Ratio_hw = 1.62 ):    
    ...
    for i in range(HS.shape[0]/2):
        for j in range(2):
            axs[i,j].plot( HS[j+2*i,:] , label = labels[j+2*i] ) ;
            axs[i,j].legend();
    # axs.set_ylabel(y_label)
    # axs.set_xlabel(x_label)
    return fig , axs

s_n_Sa  = "Sa/Histogram = {:2.2F} [GSa] ".format( l_data*n_aquisitions/(n_Hs*10.0**9) )
comment = s_n_Sa 

label_amp1 = r_[0:n_Hs].astype(str) ;
fig, axs = plot_Hist_1D_in_subfig(HS, labels = label_amp1 ,  TR_comment = comment)
fig.suptitle("both amps", fontsize=14)

# label_amp1 = r_[0:n_Hs:2].astype(str) ;
# fig, axs = plot_Hist_1D_in_subfig(HS[0::2,:], labels = label_amp1 ,  TR_comment = comment)
# fig.suptitle("1st amp", fontsize=14)

# label_amp2 = r_[1:n_Hs:2].astype(str) ;
# fig, axs = plot_Hist_1D_in_subfig(HS[1::2,:], labels = label_amp2 , TR_comment = comment)
# fig.suptitle("2nd amp", fontsize=14)

def plot_diff_Hist_1D_in_subfig( HS , labels , x_label="bin#" , y_label="Counts", TR_comment="" ,height = 10 , Ratio_hw = 1.62 ):    
    fig , axs = subplots(HS.shape[0]/2,2 ,sharey = True ,sharex = True)
    fig.set_size_inches( Ratio_hw*height , height )
    fig.text(0.74, 0.915, TR_comment , fontsize=9)
    for i in range(HS.shape[0]/2):
        for j in range(2):
            axs[i,j].plot( HS[0,:].astype('int64') - HS[j+2*i,:].astype('int64') , label = labels[j+2*i] ) ;
            axs[i,j].legend();
    # axs.set_ylabel(y_label)
    # axs.set_xlabel(x_label)
    return fig , axs
...
label_amp1 = r_[0:n_Hs].astype(str) ;
fig, axs = plot_diff_Hist_1D_in_subfig(HS, labels = label_amp1 ,  TR_comment = comment)
fig.suptitle("both amps", fontsize=14)

# label_amp1 = r_[0:n_Hs:2].astype(str) ;
# fig, axs = plot_diff_Hist_1D_in_subfig(HS[0::2,:], labels = label_amp1 ,  TR_comment = comment)
# fig.suptitle("1st amp", fontsize=14)

# label_amp2 = r_[1:n_Hs:2].astype(str) ;
# fig, axs = plot_diff_Hist_1D_in_subfig(HS[1::2,:], labels = label_amp2 , TR_comment = comment)
# fig.suptitle("2nd amp", fontsize=14)

def plot_norm_diff_Hist_1D_in_subfig( HS, labels , x_label="bin#" , y_label="Counts", TR_comment="" ,height = 10 , Ratio_hw = 1.62 ):    
    fig , axs = subplots(HS.shape[0]/2,2 ,sharey = True ,sharex = True)
    fig.set_size_inches( Ratio_hw*height , height )
    fig.text(0.74, 0.915, TR_comment , fontsize=9)
    for i in range(HS.shape[0]/2):
        for j in range(2):
            tmp = ( HS[0,:].astype('float64') - HS[j+2*i,:].astype('float64') )/ ( HS[0,:] + HS[j+2*i,:] )
            axs[i,j].plot( tmp[~numpy.isnan(tmp)] , label = labels[j+2*i] ) ;
            axs[i,j].legend();
    # axs.set_ylabel(y_label)
    # axs.set_xlabel(x_label)
    return fig , axs
...
label_amp1 = r_[0:n_Hs].astype(str) ;
fig, axs = plot_norm_diff_Hist_1D_in_subfig(HS, labels = label_amp1 ,  TR_comment = comment)
fig.suptitle("both amps", fontsize=14)

# label_amp1 = r_[0:n_Hs:2].astype(str) ;
# fig, axs = plot_norm_diff_Hist_1D_in_subfig(HS[0::2,:], labels = label_amp1 ,  TR_comment = comment)
# fig.suptitle("1st amp", fontsize=14)

# label_amp2 = r_[1:n_Hs:2].astype(str) ;
# fig, axs = plot_norm_diff_Hist_1D_in_subfig(HS[1::2,:], labels = label_amp2 , TR_comment = comment)
# fig.suptitle("2nd amp", fontsize=14)